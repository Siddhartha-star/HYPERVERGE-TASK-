...
async def get_milestone_from_db(milestone_id: int) -> Dict:
    milestone = await execute_db_operation(
        f"SELECT id, org_id, name, color FROM {milestones_table_name} WHERE id = ?", (milestone_id,), fetch_one=True
    )
    if milestone:
        return {"id": milestone[0], "org_id": milestone[1], "name": milestone[2], "color": milestone[3]}
    return None


async def get_milestone_org_id(milestone_id: int) -> int:
    org_id = await execute_db_operation(
        f"SELECT org_id FROM {milestones_table_name} WHERE id = ?", (milestone_id,), fetch_one=True
    )
    return org_id[0] if org_id else None
