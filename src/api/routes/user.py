...
from api.models import UserCourse, UserCohort, GetUserStreakResponse, NewCandidateProfileUpdate, NewCandidateProfileRead
...
@router.put("/me/profile", response_model=NewCandidateProfileRead)
async def update_my_candidate_profile(profile_data: NewCandidateProfileUpdate, user_id: int = Depends(get_current_user_id)):
    updated_profile = await update_candidate_profile(user_id, profile_data)
    if not updated_profile:
        raise HTTPException(status_code=404, detail="Candidate profile not found or no changes applied.")
    return updated_profile


@router.get("/me/profile", response_model=NewCandidateProfileRead)
