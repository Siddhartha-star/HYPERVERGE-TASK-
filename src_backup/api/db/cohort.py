...
async def add_members_to_cohort(
    ...
    admin_emails = await execute_db_operation(
        ...
        WHERE uo.org_id = ?
        AND (uo.role = 'admin' OR uo.role = 'ADMIN')
...
async def get_cohort_org_id(cohort_id: int) -> int:
    org_id = await execute_db_operation(
        f"SELECT org_id FROM {cohorts_table_name} WHERE id = ?", (cohort_id,), fetch_one=True
    )
...
async def is_user_in_cohort(user_id: int, cohort_id: int):
    output = await execute_db_operation(
        ...
        SELECT COUNT(*) > 0 FROM (
            ...
            WHERE c.id = ? AND ou.user_id = ? AND ou.role IN ('admin', 'ADMIN')
        )
        """,
