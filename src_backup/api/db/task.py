...
async def get_task_from_db(task_id: int) -> Dict:
...
async def get_task_org_id(task_id: int) -> int:
    org_id = await execute_db_operation(
        f"SELECT org_id FROM {tasks_table_name} WHERE id = ?", (task_id,), fetch_one=True
    )
