...
async def login_or_signup_user(user_data: UserLoginData) -> Dict:
    ...
    async with get_new_db_connection() as conn:
        ...
        user, is_new_user = await insert_or_return_user(
            cursor,
            user_data.email,
        ...
        # If a new user was created, also create a candidate profile for them
        if is_new_user:
            await create_candidate_profile(conn, user["id"])
